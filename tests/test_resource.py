...
def test_create_returns_201_and_delegates(
    client: TestClient, widget_service_mock: MagicMock
) -> None:
    ...
    widget_service_mock.create_widget.assert_awaited_once_with(WidgetIn(name="name", price_cents=1))


def test_create_conflict_returns_stock_409(
    client: TestClient, widget_service_mock: MagicMock
) -> None:
    """A duplicate widget name surfaces as the stock conflict problem the meta declares."""
    widget_service_mock.create_widget.side_effect = ConflictError()
...
def test_read_one_binds_path_and_delegates(
    client: TestClient, widget_service_mock: MagicMock
) -> None:
    """GET on an item route binds the id and delegates to get_widget."""
    widget_service_mock.get_widget.return_value = Widget(id="widget-id", name="name", price_cents=1)
    resp = client.get("/widgets/widget-id", headers={"authorization": "Bearer token"})
    assert resp.status_code == 200
    assert resp.json() == {"id": "widget-id", "name": "name", "priceCents": 1}
    widget_service_mock.get_widget.assert_awaited_once_with("widget-id")
...
def test_read_one_returns_parameterized_problem(
    ...
    widget_service_mock.get_widget.side_effect = WidgetNotFoundError(widget_id="widget-id")
...
def test_read_one_uses_static_custom_exception_handler(
    ...
    widget_service_mock.get_widget.side_effect = UpstreamResponseError(
        retryable=False,
    )
...
def test_read_many_returns_a_list(client: TestClient, widget_service_mock: MagicMock) -> None:
    """GET on the collection returns the service's list as a JSON array."""
    widget_service_mock.list_widgets.return_value = [
        Widget(id="widget-id", name="name", price_cents=1)
    ]
...
def test_update_full_replaces_via_put(client: TestClient, widget_service_mock: MagicMock) -> None:
    ...
    assert resp.status_code == 200
    widget_service_mock.replace_widget.assert_awaited_once_with(
...
def test_update_partial_via_patch(client: TestClient, widget_service_mock: MagicMock) -> None:
    ...
    assert resp.status_code == 200
    widget_service_mock.patch_widget.assert_awaited_once_with(
        "widget-id", WidgetPatch(price_cents=2)
    )


def test_delete_returns_confirmation(client: TestClient, widget_service_mock: MagicMock) -> None:
    ...
    widget_service_mock.delete_widget.assert_awaited_once_with("widget-id")
